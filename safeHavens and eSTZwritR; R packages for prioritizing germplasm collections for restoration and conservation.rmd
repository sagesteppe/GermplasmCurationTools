...
author: "| Reed Clark Benkendorf$^1{1,2}$, Jeremie B. Fant$$^1{1,2}$, Sophie Taddeo$^3$, Brianna Wieferich$^2$, Chris Woolridge$^2$ \n|
...
While accessioning and maintaining germplasm collections *ex situ* to develop propagules for restoration and conservation activities is cost-effective, the collection material across a species range, remains expensive due to costs associated with field work [@li2009science; CITE: expert comms?).
Hence, the sampling of germplasm from a species aims to maximize the total genetic diversity across its range, or the suitability of populations for use in restorations, while minimizing the cost of field work. 
However, systems for guiding and measuring progress relative to this principle are absent for rare taxa, and for common taxa rely on data sets that may not relate to an individual species biology or ecology (e.g., the use of provisional STZs or ecoregions), or require considerable research to develop.
...
Empirical Seed Transfer Zones, face limitations in their development as they require considerable empirical data, and given their cost are unlikely to be developed for the range of species desired by restoration ecologists (cool guy from NV I like). 
...
safeHavens partitions species ranges into geographically constrained groups by utilizing clustering of three types of distances: geographic, resistance to gene flow, or environmental, each of which has been empirically shown to influence species genetic structures [@mcrae2006isolation; @slatkin1993isolation; @wang2014isolation].
These clusters, represented by spatial data, can be used to design germplasm collection endeavors, assigning a desired number of target samples to each cluster.
...
~~~ this gets worked into introduction for safeHavens and needs to be made more concise. 
...
Clustering has used NMDS surfaces of environmental variables, or distances where each selected variables has the same weight, and ranges from use of k-means and hierarchical cluster [@crow2018provisional; @marinoni2021spatiotemporal; @shryock2017landscape; @silva2025cosst].
...
Indeed, in many of the aforementioned works supplemental materials outline their approaches in hopes that they may be transferrable, leading credence to the value of a package offering this support. 




Discussion topi: the blended need for priotitizing collections of immediately usable germplasm, while maintaining extant diversity for unforeseen alterations to climate models or conditions. Predictions are sorcery, 
...
Discussion topic: existing tools

| --------------------------------- | -------------------- | ----------------- | -------------------- | -------------------------------  |
|          Tool Name                |  Geographic Coverage |  Species Specific |  Collection Planning |              Website             |
|  Climate Restoration Toolkit      |    North America     |       No          |          No          |  climaterestorationtool.org/     |
| Native Plant Seed Mapping Toolkit |    North America     |       No          |          No          |  rconnect.usgs.gov/seed-toolkit/ |
|     Seed Lot Selection Tool       |    North America     |      Yes?         |          No          |  seedlotselectiontool.org/sst/   |

A number of more regional tools exist (e.g. https://reforestationtools.org/climate-adapted-seed-tool/) for identifying seed sources for a site, or determining seed mixes based on a variety of approaches. 
...
A vignette is available for this workstream at [Species Distribution Models](https://sagesteppe.github.io/safeHavens/articles/SpeciesDistributionModels.html). 
...
A vignette for this workstream is available at [Predictive Provenance](https://sagesteppe.github.io/safeHavens/articles/PredictiveProvenance.html). 
...
J.B.F, S.T., C.W., contributed to writing the manuscript, and made methodological improvements to the `safeHavens` software. 
...
Claude is acknowledged for assistance with coding developement of the `safeHavens` package. 

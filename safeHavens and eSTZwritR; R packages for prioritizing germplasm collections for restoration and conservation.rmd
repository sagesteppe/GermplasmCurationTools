...
However, systems for guiding and measuring progress relative to this principle are absent for rare taxa, and for common taxa rely on data sets that are unlikely to relate to an individual species biology (e.g., provisional STZs or ecoregions), or require considerable research to develop. 

Provisional Seed Transfer Zones are generally created for administrative regions (e.g, the USA), domains that can limits their coverage to only portions of a species range [@bower2014generalized; ].
Conversely, Both STZs and Ecoregions (e.g. Baileys), have a mismatch between their spatial grain, where species with small geographic ranges may fall entirely within only a few zones, offering limited guidance for making multiple collections [].  
pSTZs are generally structured from only a couple variables, which may not be correlated with those governing an individual species niche; while ecoregions capture the response of large swathes of vegetation rather than individual species. 

Accordingly, we present `safeHavens` a landscape genetics inspired package for R that helps curators develop taxon specific sampling schemes to guide attempts to maximize either the neutral genetic diversity of taxa using a variety of simple approaches. 
...
These clusters, represented by spatial data, can be used to design germplasm collection endeavors, assigning a desired number of target samples to each cluster, akin to existing SDM based approaches.
...
Approaches to better reflect the ecology of individual species have been the use of eSTZs and SDMs [].
While eSTZs, developed from molecular and/or common garden data, and modeled in the context of environmental variables, are the gold standard there development is resource intensive and unobtainable for the wide range of species desired for restoration and conservation. 
While SDMs rarely have a causal or functional relationship data available, they at least use correlations between the distribution of a species and environmental variables, and are readily produced *in silico* using pre-existing occurrence records. 
A second stage, using the species predicted suitable habitat surfaces, and the statistical model itself, is the ability to identify regions of the species range with more similarity between themselves than across them  [@crow2018provisional; @doherty2017matching; @marinoni2021spatiotemporal; @shryock2017landscape].

Species Distribution Models have repeatedly been used to identify taxon specific seed transfer zones through a two stage model fitting and clustering process [@vitt2022seed]. 
Clustering has used NMDS surfaces of environmental variables, or distances where each selected variables has the same weight, and ranges from use of k-means and hierarchical cluster [@crow2018provisional; @doherty2017matching; @marinoni2021spatiotemporal; @shryock2017landscape; @silva2025cosst].
Further variable weighing from SDM outputs are supported in the Seed Lot Selection Tool, which allows calculations of climate similarity, to identify collection gaps using the Seedlot Selection Tool, or select seed sources [@winkler2024matching; @st2022seedlot].
Indeed, many of the aforementioned works include supplemental materials outline their approaches for use, leading credence to the value of a package offering this support. 

## Usage
Functions that utilize geographic distance include: `PointBasedSample`, `EqualAreaSample` and `IBDBasedSample`, and `OpportunisticSample` which is derived from the `PointBasedSample` approach but designs additional sampling around existing collections.
Rather than optimizing designs specifically for maximizing distances directly, they partition geographic space into relatively equally sized increments that collections should be made from within.
...
This workflow seeks to identify user supplied variables associated with species occurrences, and cluster the species range into *n* specific regions using these variables and hierarchical clustering approaches, optionally the surfaces can be resampled using `PolygonBasedSample`. 
...
Discussion topic: "the question of local"  and provenance and seed mixing structures. 

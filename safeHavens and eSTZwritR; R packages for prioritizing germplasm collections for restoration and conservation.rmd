...
CHANGE
  - pSTZs fundamental mismatch in spatial domain
    - e.g. they often treat administrative areas NOT species ranges; so can not offer coverage. OR do not exist for all regions
  - pSTZs and Bailey's Ecoregions can have a fundamental mismatch in spatial grain
    - classified areas can be too coarse for smaller geographic ranged species
  - pSTZs fundamental mismatch in thematic grain 
    - fewer variables, do not map between different climate regions (e.g. tropics, through subtropics, to temperate) at possible species boundaries. 
  - Ecoregions have a mismatch in temporal grain 

ALTERNATIVES INCLUDE
  - Empirical Seed Transfer zones 
    - Genetics 
    - Common Garden 
    - Combined with environmental variables to detect putatively adaptive alleles, or phenotypes
  - Best, but expensive. (guy from NV)
  - SDM based approaches, cheap fast, can be used to guide collections. 
    - "The compass simply represents the ideal, and sight steering a compromise which allows your boat to exist at all"
...
Accordingly, we present `safeHavens` a landscape genetics inspired package for R that helps curators develop taxon specific sampling schemes to guide attempts to maximize either the neutral genetic diversity of taxa using null models or have suitable candidate material for a restoration site available.  
safeHavens partitions species ranges into geographically constrained groups by utilizing clustering of three types of distances: geographic, resistance to gene flow, or environmental, each of which has been empirically shown to influence species genetic structures, although is not always present [@mcrae2006isolation; @slatkin1993isolation; @wang2014isolation; @durka2025assessment].
...
A vignette for this workstream is available at [Species Distribution Models](https://sagesteppe.github.io/safeHavens/articles/SpeciesDistributionModels.html). 
...
A vignette for this workstream is available at [Rare Species](https://sagesteppe.github.io/safeHavens/articles/RareSpecies.html)
